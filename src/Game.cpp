  ...
  for (auto sprite : loader) {
    if (idx >= MAX_NUM_SHAPES) {
      Corrade::Utility::Warning{}
          << "More shapes than allowed. Skipping remaining shapes...";
      break;
    }
    ...
    t_renderer.reserve(
        100, (Magnum::UnsignedInt)Magnum::GL::BufferUsage::DynamicDraw);
    t_renderer.render(*m_font->createShaper(), m_font->size(),
...

  {
    for (size_t idx = 0; idx < m_num_shapes; ++idx) {
      if (m_nameChanged[idx]) {
        ...
        m_text_renderer[idx].render(*m_font->createShaper(), m_font->size(),
                                    m_spriteInnerText[idx].c_str());
        m_nameChanged[idx] = false;
      }
      m_sprites[idx]->setAlive(m_spriteAlive[idx]);
    }

    if (ImGui::BeginCombo("Select sprite",
                          m_spriteNames[m_selectedIndex].c_str())) {
      for (size_t idx = 0; idx < m_num_shapes; ++idx) {
        if (m_spriteNames[idx].empty())
          continue;
    ...
    ImGui::ColorEdit3(
        std::format("Color", m_spriteNames[m_selectedIndex]).c_str(),
        m_sprites[m_selectedIndex]->colorData());
    if (ImGui::InputText("Inner Text",
                         m_spriteInnerText[m_selectedIndex].data(), 100))
      m_nameChanged[m_selectedIndex] = true;
    ImGui::Checkbox("Alive", &m_spriteAlive[m_selectedIndex]);
  }

  for (size_t idx = 0; idx < m_num_shapes; ++idx) {
    if (auto &sprite = m_sprites[idx]; sprite->isAlive() && sprite->hasMesh()) {
      m_shader.setColor(sprite->getColor())
...
void GameEngine::viewportEvent(ViewportEvent &event) {
  ...
  m_imguiContext.relayout(Magnum::Vector2{event.windowSize()} /
                              event.dpiScaling(),
...
void GameEngine::pointerPressEvent(PointerEvent &event) {
  if (m_imguiContext.handlePointerPressEvent(event))
    return;
}

void GameEngine::pointerReleaseEvent(PointerEvent &event) {
  if (m_imguiContext.handlePointerReleaseEvent(event))
    return;
}

void GameEngine::pointerMoveEvent(PointerMoveEvent &event) {
  if (m_imguiContext.handlePointerMoveEvent(event))
    return;
}
